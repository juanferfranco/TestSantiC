...
function draw() {
  ...
  for (let layerIndex = 0; layerIndex < layers.length; layerIndex++) {
    ...
    if (
      ...
      drawCounter % (15 - layerIndex * 3) === 0 &&
      layer.length * buildingGap < width
    ) {
    ...
    for (let building of layer) {
      ...
      fill(
        col[0] * (level * colorPower),
        col[1] * (level * colorPower),
        col[2] * (level * colorPower)
      );

      rect(
        building.x,
        building.y - level * levelPower,
        building.w,
        building.h + level * levelPower
      );
    }
  }
